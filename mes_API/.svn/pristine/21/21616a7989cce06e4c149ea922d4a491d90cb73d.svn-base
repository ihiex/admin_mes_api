...
namespace SunnyMES.WebApi.Areas.MES.Controllers
{
    ...
    [ApiVersion("2.0")]
    public class KitQCController : AspNetCore.Controllers.AreaApiControllerCommon<IKitQCServices, string>
    {
        private readonly IKitQCServices iService;

        public KitQCController(IKitQCServices _iService) : base(_iService)
        {
            iService = _iService;
        }
            ...
            [FromBody] MesSnInputDto input)
        {
            ...
            try
            {
                ...
                commonResult = await FormatNGResultAsync(commonResult, listDyn, input.S_SN);
            }
            catch (Exception e)
            {
                Log4NetHelper.Error(MethodBase.GetCurrentMethod()?.DeclaringType, throwMsg, e);
