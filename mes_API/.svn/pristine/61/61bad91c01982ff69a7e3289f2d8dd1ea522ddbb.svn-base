...
public class BoxPackageAutoSimplifyRepository : MesBaseRepository, IBoxPackageAutoSimplifyRepository
{
    ...
    public BoxPackageAutoSimplifyRepository(IDbContextCore contextCore) : base(contextCore)
    {
        ...
        string S_UnitStatus, string S_URL)
    {
        ...
        if (true)
        {
            var requiredItems = poDetails.Where(x => x.Required).OrderBy(x => x.Sequence).ToList();
