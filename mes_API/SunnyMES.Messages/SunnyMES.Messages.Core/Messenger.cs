...
using SunnyMES.WeChat.CommonService.SubscribeMessage.WxApplet;
using Senparc.Weixin.Entities;
using SunnyMES.SMS.AliYun;
using SunnyMES.Commons.Core.App;

namespace SunnyMES.Messages.Application
{
            ...
            if (user != null && !string.IsNullOrEmpty(text))
            {
                ...
                if (!string.IsNullOrEmpty(template.WxAppletSubscribeTemplateId) && template.UseInWxApplet)
                {
                    ...
                    if(memberSubscribeMsg.SubscribeStatus == "accept") { 
                        UserOpenIds userOpenIds = new UserApp().GetUserOpenIdById(userId, "SunnyMES.openid.wxapplet");
                        if (userOpenIds != null )
                        {
                            ...
                            result.ResultCode = wxJsonResult.errcode.ToString();
                            result.ResultMsg = wxJsonResult.errmsg;
                        }
                    }
                    else
                    {
                        result.Success = true;
                        result.ResultCode = "用户拒绝订阅";
                    }
                }
                else
                {
                    result.Success = true;
                    result.ResultCode = "用户未订阅";
                }
                    ...
                    if (memberSubscribeMsg!=null)
                    {
                        if (memberSubscribeMsg.SubscribeStatus == "accept")
                        {
                            ...
                            if (userOpenIds != null)
                            {
                                ...
                                {
                                    result.ResultCode = "0";
                                    result.ResultMsg = wxJsonResult.errcode.ToString();
                                }
                                else
                                {
                                    result.ResultCode = "0";
                                    result.ResultMsg = wxJsonResult.errcode.ToString() + wxJsonResult.ToJson();
                                }
                            }
                        }
                        else
                        {
                            result.ResultCode = "0";
                            result.ResultMsg = "用户拒绝";
                        }
                    }
                    else
                    {
                        result.ResultCode = "0";
                        result.ResultMsg = "用户未订阅";
                    }
                }
                else
                {
