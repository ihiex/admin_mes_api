...
using SunnyMES.WebApi.Areas.WeiXin.Models;
using SunnyMES.WeChat.Model;
namespace SunnyMES.WebApi.Areas.Weixin.Controllers
{
    /// <summary>
    /// 微信接口
    /// </summary>
    ...
    public class WxOpenController : ApiController
    {
       ...
       /// <param name="_roleService"></param>
        ...
        public IActionResult OnLogin(string code)
        {
            ...
            try
            {
                ...
                if (jsonResult.errcode == ReturnCode.请求成功)
                {
                    ...
                    User user = userService.GetUserByOpenId("SunnyMES.openid.wxapplet", jsonResult.openid);
                    if (user == null)
                    {
                        UserInputDto userInput = new UserInputDto();
                        userInput.OpenId = jsonResult.openid;
                        user.UnionId = jsonResult.unionid;
                        userInput.OpenIdType = "SunnyMES.openid.wxapplet";
                    ...
                    if (user == null)
                    {
                        user = userService.GetUserByOpenId("SunnyMES.openid.wxapplet", jsonResult.openid);
                    }
                ...
                else
                {
                    ...
                    result.ResultMsg = jsonResult.errmsg;
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> UpdatePhone(string sessionId, string tell)
        {
            ...
            try
            {
                User user = userService.GetUserByOpenId("SunnyMES.openid.wxapplet",sessionBag.OpenId);
        ...
        public async Task<IActionResult> GetReferralQrCode()
        {
            ...
            try
            {
                ...
                {
                    result.ResultCode = resultImg.errcode.ToString();
                    result.ResultMsg = resultImg.errmsg;
                }
            }
            catch (Exception ex)
            {
                Log4NetHelper.Error("代码生成异常", ex);
                result.ResultMsg = "代码生成异常:" + ex.Message;
                result.ResultCode = ErrCode.failCode;
            }
            return ToJsonContent(result);
        ...
        public async Task<IActionResult> ContentWxAppletQrCode([FromBody]ContentPlayBillModel info)
        {
            ...
            try
            {
                ...
                {
                    result.ResultCode = ErrCode.successCode;
                    result.ResultMsg = _tempfilepath + picname;
                }
        ...
        public IActionResult QuikLogin(WxUserInfo info)
        {
            ...
            }catch(Exception ex)
            {
                ...
                result.ResultMsg = "微信快速（一键）登录:" + ex.Message;
                result.ResultCode = ErrCode.failCode;
            }
            return ToJsonContent(result);
        }
        ...
        public IActionResult LoginByOpenId(string openId)
        {
            ...
            {
                YuebonCacheHelper yuebonCacheHelper = new YuebonCacheHelper();
                User user = userService.GetUserByOpenId("SunnyMES.openid.wxapplet", openId);
                if (user == null)
                {
                    UserInputDto userInput = new UserInputDto();
                ...
                if (user == null)
                {
                    user = userService.GetUserByOpenId("SunnyMES.openid.wxapplet", openId);
                }
            ...
            catch (Exception ex)
            {
                ...
                result.ResultMsg = "微信登录异常:" + ex.Message;
                result.ResultCode = ErrCode.successCode;
            }
        ...
        public IActionResult CheckOpenId(string openId)
        {
            ...
            catch (Exception ex)
            {
                ...
                result.ResultMsg = "检查微信用户的OpenId:" + ex.Message;
                result.ResultCode = ErrCode.failCode;
            }
            return ToJsonContent(result);
        }
        ...
        public IActionResult ImgSecCheck(string filePath)
        {
            ...
            try
            {
                ...
                {
                    result.ResultCode = ErrCode.failCode;
                    result.ResultMsg = "图片含有违法违规内容，请更换图片";
                    result.ResData = res;
                }
            }
            catch (Exception ex)
