...
using SunnyMES.WebApi.Areas.MES.Controllers;
...
namespace SunnyMES.WebApi.Areas.Security.Controllers
{
    ...
    [ApiVersion("2.0")]
    public class ChangePOPNController : AreaApiControllerReport<IChangePOPNServices, string>
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="_iService"></param>
        public ChangePOPNController(IChangePOPNServices _iService) : base(_iService)
        {
            iService = _iService;
        }
        ...
        /// <returns></returns>
        [HttpGet("SetConfirmPO_ChangePOPN")]
        [YuebonAuthorize("")]
        [AllowAnonymousAttribute]
        public async Task<IActionResult> SetConfirmPO_ChangePOPN(string S_PartFamilyTypeID, string S_PartFamilyID,
            string S_PartID, string S_POID, string S_UnitStatus, string S_URL)
        {
            ...
            try
            {
                ...
                ConfirmPOOutputDto List_Dyn = await iService.SetConfirmPO_ChangePOPN(S_PartFamilyTypeID, S_PartFamilyID,
        ...
        /// <param name="S_PartID_Target">目标料号</param>
        /// <param name="S_POID_Target">目标工单</param>
        /// <param name="S_COF">COF  SetConfirmPO_ChangePOPN 此方法中返回有COF值 </param>
        /// <param name="S_URL">URL</param>
        /// <returns></returns>
        [HttpGet("SetScanSN_ChangePOPN")]
        [YuebonAuthorize("")]
        [AllowAnonymousAttribute]
        public async Task<IActionResult> SetScanSN_ChangePOPN(string S_SN, string S_PartFamilyTypeID, string S_PartFamilyID,
                    ...
                    string S_PartID_Target, string S_POID_Target, string S_COF, string S_URL)
        {
            ...
            try
            {
                ...
                SetScanSNOutputDto List_Dyn = await iService.SetScanSN_ChangePOPN(S_SN, S_PartFamilyTypeID, S_PartFamilyID,
                    S_PartID, S_POID, S_UnitStatus, S_DefectID, S_PartID_Target, S_POID_Target, S_COF, S_URL);

                if (List_Dyn.MSG.ValStr1 == "1")
                {
                    v_ComResult.Success = true;
                    v_ComResult.ResultCode = ErrCode.successCode;
                    v_ComResult.ResultMsg = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " " + ErrCode.err0;
                    v_ComResult.ResData = List_Dyn;
                    v_ComResult.Sounds = S_Path_OK;
                }
                else
