...
namespace SunnyMES.Security.Application
{
    ...
    public class MenuApp
    {
        ...
        /// <returns></returns>
        public List<MenuOutputDto> GetMenuFuntionJson(string roleIds, string systemCode,string UserId)
        {
            List<MenuOutputDto> list = new List<MenuOutputDto>();
            try
            {
                SystemType systemType = systemservice.GetByCode(systemCode); 
                list = GetMenusByRole(roleIds, systemType.Id, UserId).OrderBy(t => t.SortCode).MapTo<MenuOutputDto>();
            }
            catch (Exception ex)
            {
        ...
        /// <param name="systemCode">系统类型代码子系统代码</param>
        /// <returns></returns>
        public List<VueRouterModel> GetVueRouter(string roleIds, string systemCode,string UserId)
        {
            List<VueRouterModel> list = new List<VueRouterModel>();
            try
            { 
                SystemType systemType = systemservice.GetByCode(systemCode);
                List<Menu> listMenu = GetMenusByRole(roleIds, systemType.Id, UserId).OrderBy(t => t.SortCode).ToList();
        ...
        public List<VueRouterModel> BuildMenus(List<MenuOutputDto> menus)
        {
            ...
            foreach (MenuOutputDto menu in menus)
            {
                ...
                Meta meta=  new Meta(menu.FullName, menu.Icon == null ? "" : menu.Icon, menu.IsCache
                        ,menu.IsShow,menu.IsFrame,Convert.ToBoolean( menu.IsPublic),menu.EnabledMark
                    );
        ...
        /// <returns></returns>
        public List<Menu> GetMenusByRole(string roleIds, string systemId,string UserId)
        {
            List<Menu> menuListResult = new List<Menu>();
            if (roleIds == "")
            {
               menuListResult = service.GetFunctions("", systemId, UserId, true);
            }
            else
            {
                string roleIDsStr = string.Format("'{0}'", roleIds.Replace(",", "','"));
                menuListResult = service.GetFunctions(roleIDsStr, systemId, UserId, true);
            }
        ...
        /// <returns></returns>
        public List<MenuOutputDto> GetFunctionsByUser(string userID, string typeID,string RoleId)
        {
            string where = string.Format("");
            string roleId = RoleId;//serviceUser.Get(userID).RoleId;
            List<MenuOutputDto> functions = new List<MenuOutputDto>();
            string roleIDsStr = string.Format("'{0}'", roleId.Replace(",", "','"));
            if (roleIDsStr != "")
            {
                functions = service.GetFunctions(roleIDsStr, typeID, userID).ToList().MapTo<MenuOutputDto>();
            }
            return functions;
        }
        /// <summary>
        ...
        /// <param name="systemId">系统类型ID/子系统ID</param>
        /// <returns></returns>
        public List<MenuOutputDto> GetFunctionsByRole(string roleIds, string systemId,string UserId)
        {
            string where = string.Format("");
            List<MenuOutputDto> functions = new List<MenuOutputDto>();
            string roleIDsStr = string.Format("'{0}'", roleIds.Replace(",", "','"));
            if (roleIDsStr != "")
            {
                functions = service.GetFunctions(roleIDsStr, systemId, UserId).ToList().MapTo<MenuOutputDto>();
            }
            return functions;
        }
