...
namespace SunnyMES.Commons.Core.DataManager
{
    ...
    public class DBServerProvider
    {
        ...
        {          
            ...
            {
                string S_Conn = Configs.GetConfigurationValue("DbConnections:" + dbConfigName + ":MasterDB", "ConnectionString");
                string S_JM=  SFCEncode.DecryptPassword(S_Conn, "");
                if (S_JM == "-1:ciphertext is wrong") 
                {
                    S_JM = S_Conn;
                }
                dbConnectionOptions.ConnectionString = S_JM;
        ...
        /// <param name="masterDb">是否访问主库，默认为是，否则访问从库即只读数据库</param>
        /// <returns></returns>
        public static DbConnectionOptions GeDbConnectionOptions2(bool masterDb = true)
        {
