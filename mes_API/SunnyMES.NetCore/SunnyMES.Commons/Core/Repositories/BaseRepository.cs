...
namespace SunnyMES.Commons.Repositories
{
        ...
        where T : Entity
    {
        ...
        public virtual List<T> FindWithPager(string condition, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            ...
            PagerHelper pagerHelper = new PagerHelper(this.tableName, this.selectedFields, fieldToSort, info.PageSize, info.CurrentPageIndex, desc, condition);
        ...
        public virtual async Task<bool> DeleteBatchWhereAsync(string where, IDbTransaction trans = null)
        {
            ...
            var param = new List<Tuple<string, object>>();
            string sql = $"delete from {tableName} where " + where;
            if (tableName == "mesEmployee") 
            {
                string S_UserLogon = where.Replace("Id", "UserId");
                S_UserLogon = S_UserLogon.Replace("id", "UserId");
                S_UserLogon = S_UserLogon.Replace("ID", "UserId");                

                sql += "\r\n" + " delete API_UserLogOn where "+ S_UserLogon;
            }
        ...
        string S_ConnSqLite = "Data Source=System.MES.db;Pooling=true;FailIfMissing=false;";
        /// <summary>
        /// ExecSQLite
        /// </summary>
        /// <param name="S_SQLite"></param>
        /// <returns></returns>
        public string ExecSQLite(string S_SQLite)
        {
            string S_Result = "OK";
            SQLiteConnection Conn = new SQLiteConnection(S_ConnSqLite);
            Conn.Open();
            try
            {
                SQLiteCommand myCommand = new SQLiteCommand(S_SQLite, Conn);
        ...
        /// <param name="S_SQLite"></param>
        /// <returns></returns>
        public DataTable SQLite_Data_Table(string S_SQLite)
        {
            ...
            try
            {
                SQLiteConnection Conn = new SQLiteConnection(S_ConnSqLite);
                Conn.Open();
                SQLiteCommand cmd = new SQLiteCommand(S_SQLite, Conn);
                SQLiteDataAdapter SQLite_DA = new SQLiteDataAdapter(cmd);
                SQLite_DA.Fill(DT);
                SQLite_DA.Dispose();
                Conn.Close();
            }
        ...
        public DataTable Data_Table(string S_Sql)
        {
            ...
            try
            {
                ...
                System.Data.SqlClient.SqlDataAdapter Sql_DA = new System.Data.SqlClient.SqlDataAdapter(cmd);
                Sql_DA.Fill(DT);
                Sql_DA.Dispose();
                Conn.Close();
            }
