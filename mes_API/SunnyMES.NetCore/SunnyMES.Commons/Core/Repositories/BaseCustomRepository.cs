...
namespace SunnyMES.Commons.Repositories
{
    ...
    public abstract class BaseCustomRepository<T, TKey> : ICustomRepository<T, TKey>, ITransientDependency
        where T : Entity
    {
        /// <summary>
        /// 数据库超时(10分)  600000 秒
        ...
        ///  EF DBContext
        /// </summary>
        public IDbContextCoreCustom _dbContext;
        ...
        public BaseCustomRepository(IDbContextCoreCustom dbContext)
        {
        ...
        public BaseCustomRepository(IDbContextFactory dbContextFactory)
        {
        ...
        protected CommonHeader customHead = new CommonHeader();
        ...
        protected async Task FormatCloneMsg(T beforeT, T cloneT)
        {
            ...
            {
                CurrentIP = customHead.CurrentLoginIp,
                DataType = "Clone",
                TableName = tableName,
                UserName = customHead.UserFullName,
                MSG = MSG
            });
        }
        protected async Task FormatDeleteMsg(T mainT, params object[] childs)
        {
            string MSG =
               mainT.ToJson() + "\r\n" +
               childs.ToJson()
        ...
        public virtual void SetCommonHeader(CommonHeader commonHeader)
        {
            this.customHead = commonHeader;
        }
        #region Dapper 操作
        ...
        public virtual  int GetMaxID()
        {
            ...
            int NextID = DapperConn.ExecuteScalar<int>(sql);
            return NextID;
        }
        ...
        public virtual List<T> FindWithPager(string condition, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            ...
            PagerHelper pagerHelper = new PagerHelper(this.tableName, this.selectedFields, fieldToSort, info.PageSize, info.CurrentPageIndex <= 0 ? 1 : info.CurrentPageIndex, desc, condition);
        ...
        public virtual async Task<List<T>> FindWithPagerAsync(string condition, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            ...
            PagerHelper pagerHelper = new PagerHelper(this.tableName, this.selectedFields, string.IsNullOrEmpty(fieldToSort)?primaryKey:fieldToSort, info.PageSize, info.CurrentPageIndex <= 0 ?1:info.CurrentPageIndex, desc, condition);
        ...
        public virtual List<T> FindWithPagerSql(string condition, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            ...
            int startRows = (info.CurrentPageIndex - 1) * info.PageSize + 1;//起始记录
            int endNum = info.CurrentPageIndex * info.PageSize;//结束记录
        ...
        /// <returns></returns>
        public virtual async Task<IEnumerable<T>> FindWithAllPagerAsync(PageCustomInfo pageCustomInfo)
        {
            //string sql = $"SELECT * FROM {tableName} order by {(string.IsNullOrEmpty(pageCustomInfo.Sortfield)? primaryKey: pageCustomInfo.Sortfield)} {(pageCustomInfo.IsAsc ? "ASC" : "DESC")}";
            string sql = $"SELECT * FROM {tableName} { (string.IsNullOrEmpty( (string.IsNullOrEmpty(pageCustomInfo.Sortfield) ? primaryKey : pageCustomInfo.Sortfield).Trim()) ? "" : $"order by {(string.IsNullOrEmpty(pageCustomInfo.Sortfield) ? primaryKey : pageCustomInfo.Sortfield)} {(pageCustomInfo.IsAsc ? "ASC" : "DESC")}")}";
            return await DapperConn.QueryAsync<T>(sql);
        }
        ...
        /// <param name="trans"></param>
        /// <returns></returns>
        public virtual async Task<List<T>> FindWithPagerCustomSqlAsync(string sql, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            PagerCustomHelper pagerHelper = new PagerCustomHelper(sql, this.selectedFields, string.IsNullOrEmpty(fieldToSort) ? primaryKey : fieldToSort, info.PageSize, info.CurrentPageIndex <= 0 ? 1 : info.CurrentPageIndex, desc);
        ...
        public virtual async Task<List<W>> FindWithPagerCustomSqlAsync<W>(string sql, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            PagerCustomHelper pagerHelper = new PagerCustomHelper(sql, this.selectedFields, string.IsNullOrEmpty(fieldToSort) ? primaryKey : fieldToSort, info.PageSize, info.CurrentPageIndex <= 0 ? 1 : info.CurrentPageIndex, desc);
            ...
            list = reader.Read<W>().ToList();
            return list;
        }
        public virtual List<W> FindWithPagerCustomSql<W>(string sql, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            PagerCustomHelper pagerHelper = new PagerCustomHelper(sql, this.selectedFields, string.IsNullOrEmpty(fieldToSort) ? primaryKey : fieldToSort, info.PageSize, info.CurrentPageIndex <= 0 ? 1 : info.CurrentPageIndex, desc);
            string pageSql = pagerHelper.GetSqlServerCustomComplexSql();
            var reader =  DapperConnRead.QueryMultiple(pageSql);
        ...
        public virtual List<object> FindWithPagerRelationUser(string condition, PagerInfo info, string fieldToSort, bool desc, IDbTransaction trans = null)
        {
            ...
            int endNum = info.CurrentPageIndex * info.PageSize;//结束记录
            string strOrder = string.Format(" {0} {1}", fieldToSort, desc ? "DESC" : "ASC");
            sb.AppendFormat("SELECT count(*) as RecordCount FROM (select t1.{0} FROM {1} t1 inner join API_User t2 on t1.CreatorUserId = t2.Id where {2})  AS main_temp;", primaryKey, tableName, condition);
            sb.AppendFormat("SELECT * FROM (SELECT ROW_NUMBER() OVER (order by  {0}) AS rows ,t1.{1},t2.Account as Account,t2.NickName as NickName,t2.RealName as RealName,t2.HeadIcon as HeadIcon ,t2.MobilePhone as MobilePhone  FROM {2} t1 inner join API_User t2 on t1.CreatorUserId = t2.Id " +
        ...
        public virtual long Insert(T entity, IDbTransaction trans = null)
        {
            if (entity.KeyIsNull())
            {
                entity.GenerateDefaultKeyVal(GetMaxID());
            }
            long L_Result = DbContext.Add<T>(entity);
        ...
        public virtual async Task<long> InsertAsync(T entity, IDbTransaction trans = null)
        {
            ...
            long result = _dbContext.Add(entity);
            return result;
        }
        ...
        public virtual async Task<long> UpdateAsync2(T entity, TKey primaryKey, IDbTransaction trans = null)
        {
        ...
        /// EF 上下文接口，可读可写
        /// </summary>
        public virtual IDbContextCoreCustom DbContext
        {
        ...
        public virtual IEnumerable<T> GetByPagination(Expression<Func<T, bool>> @where, PagerInfo pagerInfo, bool asc = false, params Expression<Func<T, object>>[] @orderby)
        {
            ...
            return filter.Skip(pagerInfo.PageSize * (pagerInfo.CurrentPageIndex - 1)).Take(pagerInfo.PageSize);
        }
        /// <summary>
        /// sql语句查询数据集
        /// </summary>
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IDbTransaction GetDbTran() => DapperConn.BeginTransaction();
