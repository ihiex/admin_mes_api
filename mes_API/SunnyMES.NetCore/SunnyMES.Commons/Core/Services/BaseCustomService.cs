...
namespace SunnyMES.Commons.Services
{
    ...
    public abstract class BaseCustomService<T, TODto, TKey> : ICustomService<T, TODto, TKey>, ITransientDependency
        where T : Entity
        where TODto : class
        where TKey : IEquatable<TKey>
    {
        ...
        /// 
        /// </summary>
        protected ICustomRepository<T, TKey> repository;
        ...
        protected BaseCustomService(ICustomRepository<T, TKey> iRepository)
        {
        ...
        protected BaseCustomService(ICustomRepository<T, TKey> iRepository, IHttpContextAccessor accessor)
        {
            _accessor = accessor;
            repository = iRepository;

        }
        public BaseCustomService()
        {
        ...
        public virtual void SetCommonHeader(CommonHeader commonHeader)
        {
            this.commonHeader = commonHeader;
             repository.SetCommonHeader(commonHeader);
        }
        public virtual IDbTransaction GetDbTran()
        {
            return repository.GetDbTran();
        }
        ...
        public virtual Task<long> UpdateAsync2(T entity, TKey id, IDbTransaction trans = null)
        {
            return repository.UpdateAsync2(entity, id, trans);
        ...
        {
            return await repository.FindWithAllPagerAsync(pageCustomInfo);
        }
        /// <summary>
        /// 根据条件查询数据库,并返回对象集合(用于分页数据显示)
        /// </summary>
