...
namespace SunnyMES.Commons.Helpers
{
    ...
    public class FileHelper
    {
        ...
        /// <param name="targatePath"></param>
        /// <param name="isReplease"></param>
        public static void MoveFile(string sourceFilePath, string targatePath,  bool isDeleteSourceFile = false)
        {
            string result = string.Empty;
            string dicPath = sourceFilePath.ToFilePath();
            string fileName = new FileInfo(sourceFilePath).Name;
            string targetFilePath = Path.Combine(targatePath, fileName);

            if (!Directory.Exists(targatePath))//如果不存在就创建file文件夹
            {
                Directory.CreateDirectory(targatePath);
            }
            File.Move(sourceFilePath,targetFilePath,true);
            //File.Copy(targatePath, targetFilePath, true);
            if (isDeleteSourceFile) {File.Delete(sourceFilePath); }
        }
