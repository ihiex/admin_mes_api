...
namespace SunnyMES.Commons.Helpers
{
    ...
    public class CSVHelper
    {
        ...
        /// <param name="csvPath">csv文件路径</param>
        /// <returns></returns>
        public static DataTable CSVToDataTableByStreamReader(MemoryStream memoryStream)
        {
            DataTable csvdt = new DataTable("csv");
        ...
        /// <param name="csvPath">csv文件路径</param>
        /// <returns></returns>
        public static List<T> CSVToDataTableByStreamReader<T>(MemoryStream memoryStream)
        {
            List<T> csvdt = new List<T>();
            ...
            using (StreamReader reader = new StreamReader(memoryStream))
            {
                while (!string.IsNullOrEmpty((strline = reader.ReadLine())))
                {
                    ...
                    for (int i = 0; i < intColCount; i++)
                    {
                        var item = type.FirstOrDefault(m => m.Name == colomns[i]);
                        if (item is null)
                            continue;

                        if (string.IsNullOrEmpty(aryline[i].Trim()))
                            continue;
                        item.SetValue(t, ConvertHelper.ConvertTo( aryline[i], item.PropertyType));
                    }
                    csvdt.Add(t);
                }
            }
