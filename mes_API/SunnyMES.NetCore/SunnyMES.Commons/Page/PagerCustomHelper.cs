...
namespace SunnyMES.Commons.Pages
{
    ...
    /// </summary> 
    public class PagerCustomHelper
    {
        #region 成员变量
        ...
        public PagerCustomHelper(string tableName, string fieldsToReturn, string fieldNameToSort,
        ...
        public string GetSqlServerCustomSql(bool isSql2012 = false)
        {
            string sql = "";
            if (string.IsNullOrEmpty(this.strwhere))
            {
            ...
            sql = $@"
                    ...
                    END
                    select * into {tmpTableName} from {this.TableOrSqlWrapper} Where {this.strwhere}
                    select count(*) as Total from {tmpTableName}
";

            if (isSql2012)
            {
                sql += $"SELECT * FROM ( SELECT ROW_NUMBER() OVER (order by {strOrder}) AS rows ,{fieldsToReturn} FROM {tmpTableName}) AS main_temp where rows BETWEEN {minRow} and {maxRow};";
            }
            else
            {
                sql += $"SELECT * FROM {tableName}  order by {fieldNameToSort} {(this.isDescending ? "DESC" : "ASC")} offset (({pageIndex} - 1) * {pageSize}) rows fetch next {pageSize} rows only;";
            }
            return sql;
        }
        ...
        /// <returns></returns>
        public string GetSqlServerCustomComplexSql( bool isSql2012 = false)
        {
            string sql = "";
            if (string.IsNullOrEmpty(this.strwhere))
            {
            ...
            sql = $@"
                    ...
                    BEGIN
                        DROP TABLE {tmpTableName}
                    END
                    select * into {tmpTableName} from {this.TableOrSqlWrapper} 
                    select count(*) as Total from {tmpTableName}
";
            ...
            {
                sql += $"SELECT * FROM {tmpTableName}  order by {fieldNameToSort} {(this.isDescending ? "DESC" : "ASC")} offset (({pageIndex} - 1) * {pageSize}) rows fetch next {pageSize} rows only;";
            }
            return sql;
        }
